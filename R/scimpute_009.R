...
find_neighbors_labeled = function(count_hv, J, Kcluster = NULL, ncores,
    ...
    nclust = length(unique(clust)); print("calculating cell distances ...")
...
find_neighbors_unlabeled <- function(count_hv, J, Kcluster = NULL, ncores,
    cell_labels = NULL) {
    ...
    dist_cells_list = parallel::parLapply(cl, seq_len(J), function(id1) {
        d = vapply(seq_len(id1), function(id2) {
            sse = sum((mat_pcs[, id1] - mat_pcs[, id2])^2)
            sqrt(sse) }, FUN.VALUE = 1)
        return(c(d, rep(0, J - id1))) } ); parallel::stopCluster(cl)
...
get_mix_parameters <- function(count, point = log10(1.01), path, ncores = 8) {
    ...
    parslist = parallel::parLapply(cl, seq_len(nrow(count)), function(ii) {
        ...
        xdata = count[ii, ]
        paramt = tryCatch(get_mix(xdata, point), error = function(e) rep(NA,
            5))
        return(paramt)
    })
    parallel::stopCluster(cl)
...
imputation_model8 = function(count, labeled = FALSE, point, drop_thre = 0.5,
    ...
    if (Kcluster == 1) { clust = rep(1, J)
        ...
        for (cellid in seq_len(J)) {
            dist_cells[cellid, ] = dist_cells_list[[cellid]] }
        dist_cells = dist_cells + t(dist_cells)
    } else { print("inferring cell similarities ...")
    ...
    return(MixtureModel(count, clust, ncores, drop_thre))
}
...
imputation_wlabel_model8 = function(count, labeled, cell_labels = NULL, point,
    ...
    for (cc in seq_len(nclust)) {
        ...
        parslist <- get_mix_parameters(count = count[, which(clust == cc),
            drop = FALSE], point = log10(1.01), ncores = ncores)
